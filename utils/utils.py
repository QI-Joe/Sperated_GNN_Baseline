...
def convert2pyg_batch_data(data_list: list[Data], node_feature, edge_feature, task = "link") -> Temporal_Dataloader:
    new_data_list = list()
    
    for data in data_list:
        pyg_data = convert2pyg_data(data, node_feature, edge_feature, task)
        new_data_list.append(pyg_data)
    
    return new_data_list

def convert2pyg_data(custom_data: Data, n_feat, e_feat, task = "link") -> Temporal_Dataloader:
    """
    Convert your custom Data object to PyG Data for NeighborLoader.
    """

    # 1. Build node id to local index mapping to zero-based consecutive node indices
    src = custom_data.src_node_ids
    dst = custom_data.dst_node_ids
    unique_nodes = np.unique(np.concatenate([src, dst]))
    node_id_map = {nid: i for i, nid in enumerate(unique_nodes)}

    # Use np.vectorize to map src and dst to local indices efficiently
    map_func = np.vectorize(lambda nid: node_id_map[nid])
    src_local = map_func(src)
    dst_local = map_func(dst)
    
    edge_index = torch.from_numpy(np.vstack([src_local, dst_local])).type(torch.long)
    edge_ids = custom_data.edge_ids
    interact_times = custom_data.node_interact_times
    # 3. Node features: if provided, reorder according to unique_nodes
    if task == "link":
        label = torch.arange(len(unique_nodes))
    elif task == "node":
        label = (torch.from_numpy(custom_data.robustness_match_tuple[0]), torch.from_numpy(custom_data.robustness_match_tuple[1]))
    node_feat = n_feat[unique_nodes]
    edge_feat = e_feat[edge_ids]

    # 6. Create PyG Data object
    pyg_data = PyGData(
        x=torch.tensor(node_feat, dtype=torch.float32),
        edge_index=edge_index,
        edge_attr=torch.tensor(edge_ids, dtype=torch.long),
        time = torch.tensor(interact_times, dtype=torch.float32),
        y=label,
        pos=torch.tensor(edge_feat, dtype=torch.float32),
    )
    return pyg_data