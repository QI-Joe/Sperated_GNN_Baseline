...
if __name__ == "__main__":
    ...
    node_cls, node_raw_features, edge_raw_features, data_list = \
        get_node_classification_data(dataset_name=args.dataset_name, snapshot=SNAPSHOT, task="None", val_ratio=args.val_ratio, test_ratio=args.test_ratio)
    ...
    for run in range(1):
        ...
        for epoch in range(args.num_epochs):
            ...
            for batch_idx, batch_data in enumerate(train_idx_data_loader_tqdm):
                ...
                recovered_nodeidx = batch_data.n_ids[:batch_data.batch_size]
                node_mask = torch.isin(recovered_nodeidx, node_allow2see)
                
                predicts = model[1].forward(x=batch_z1)[:batch_data.batch_size][node_mask]
                labels = full_label[recovered_nodeidx][node_mask]
                
                loss = loss_func(input=predicts, target=labels)
                train_losses.append(loss.item())

                train_metrics.append(get_node_classification_metrics(predicts=predicts, labels=labels))
