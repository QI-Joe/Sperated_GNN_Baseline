...
from evaluate_models_utils import evaluate_model_link_prediction, evaluate_contrastive_model_link_prediction
        ...
        for epoch in range(args.num_epochs):
            ...
            for batch_idx, batch_data in enumerate(train_idx_data_loader_tqdm):
                ...
                if args.model_name == "GCA":                    
                    ...
                    batch_src_node_embeddings = batch_z1[batch_data.edge_index[0]]
                    batch_dst_node_embeddings = batch_z1[batch_data.edge_index[1]]                    
                ...
                batch_neg_src_node_ids = torch.from_numpy(batch_src_node_ids).to(device)
                batch_neg_src_node_embeddings = batch_z1[batch_neg_src_node_ids]
                batch_neg_dst_node_embeddings = batch_z1[batch_neg_dst_node_ids]
            ...
            val_losses, val_metrics = evaluate_contrastive_model_link_prediction(model_name=args.model_name,
                                                                    model=model,
                                                                    evaluate_idx_data_loader=val_idx_data_loader,
                                                                    loss_func=loss_func, mvgrl_loss=mvgrl_loss)

            new_node_val_losses, new_node_val_metrics = evaluate_contrastive_model_link_prediction(model_name=args.model_name,
                                                                                       model=model,
            ...
            if (epoch + 1) % args.test_interval_epochs == 0:
                test_losses, test_metrics = evaluate_contrastive_model_link_prediction(model_name=args.model_name,
                                                                           model=model,
                                                                           evaluate_idx_data_loader=test_idx_data_loader,
                                                                           loss_func=loss_func, mvgrl_loss=mvgrl_loss)


                new_node_test_losses, new_node_test_metrics = evaluate_contrastive_model_link_prediction(model_name=args.model_name,
                                                                                             model=model,
